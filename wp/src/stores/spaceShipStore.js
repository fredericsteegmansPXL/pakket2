...
export const useSpaceShipStore = defineStore("spaceShipStore", () => {
  ...
  const totalPrice = computed(() => {
    return productIdsInCart.value.reduce((acc, value) => {
      const product = allProducts.value.find((prod) => prod.id === value.id);
      return acc + product.prijs * value.amount;
    }, 0);
  });
  ...
  const getProductsInCart = computed(() => {
    ...
    return [...ids].reduce((acc, value) => {
      ...
      const amount = productIdsInCart.value.find(
        (prod) => prod.id === value
      )?.amount;
  ...
  const getCartCount = computed(() =>
    productIdsInCart.value.reduce((acc, value) => acc + value.amount, 0)
  );
  ...
  function removeFromCart(id) {
    ...
    if (item && item.amount > 1) {
      ...
      productIdsInCart.value = productIdsInCart.value.filter(
        (prod) => prod.id !== id
      );
    }
  }
